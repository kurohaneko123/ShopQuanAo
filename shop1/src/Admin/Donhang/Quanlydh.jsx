...
export default function Quanlydh() {
  ...
  const submitEditInfo = async () => {
    ...
    try {
      ...
      if (!payload.tennguoinhan || !payload.sodienthoai || !payload.diachigiao) {
        return Swal.fire("Thiếu dữ liệu", "Vui lòng nhập đủ thông tin", "warning");
      }
                ...
                <tr key={x.madonhang} className="border-b border-white/5">
                  ...
                  <td className="p-3">
                    <div className="grid grid-cols-4 gap-2 place-items-center">
                      ...
                      {/* CHECK HOÀN TIỀN */}
                      <button
                        className={`${actionBtn(disableRefundCheck)} bg-sky-600`}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-[#111] w-[600px] p-6 rounded-xl border border-white/10">
            <h3 className="text-xl font-bold mb-4">
              {isEdit ? "Sửa đơn hàng" : "Chi tiết đơn hàng"} #{selectedOrder.madonhang}
            </h3>

            {!isEdit ? (
              <>
                <p><b>Người nhận:</b> {selectedOrder.tennguoinhan}</p>
                <p><b>SĐT:</b> {selectedOrder.sodienthoai}</p>
                <p><b>Địa chỉ:</b> {selectedOrder.diachigiao}</p>
                <p><b>Thanh toán:</b> {selectedOrder.hinhthucthanhtoan}</p>
                <p><b>Ghi chú:</b> {selectedOrder.ghichu || "Không có"}</p>
